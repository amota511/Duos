...
//  Duos V2
//
//  Created by amota511 on 8/21/15.
...
class levelEndless: SKScene, SKPhysicsContactDelegate {
    ...
    var gameView5: GameViewController!
    ...
    override func didMove(to view: SKView) {
        ...
        let skyColor1 =  SKColor(red: 0xff/255.0, green: 195.0/255.0, blue: 115.0/255.0, alpha: 1.0)
        ...
        let ScreenColor: SKColor! = tapColor[1]
        ...
        topBar.position = CGPoint(x: 0,y: 22)
        ...
        let delayShape = SKAction.wait(forDuration: TimeInterval(timeLimit))
        let spawnThenDelayShape = SKAction.sequence([spawnShapes,delayShape])
        let spawnThenDelayShapeForever = SKAction.repeatForever(spawnThenDelayShape)
        ...
        //var currentScore = scores.doubleForKey("Score")
        if (score == score) {
            scores.set(score, forKey: "Score")
        }
        ...
        scoreLabelNode.text = String("\(score) / \(showHighScore)")
    ...
    func riseBlackBall() {
        self.blackBall.physicsBody?.applyForce(CGVector(dx: 0.0,dy: 17.67))
    }
    
    func riseGreyBall() {
        self.greyBall.physicsBody?.applyForce(CGVector(dx: 0.0,dy: -17.67))
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if moving.speed > 0 {
            for touch: AnyObject in touches {
                ...
                blackBall.physicsBody?.velocity = CGVector(dx: 0,dy: -self.frame.size.height / 3.66)
                greyBall.physicsBody?.velocity = CGVector(dx: 0,dy: self.frame.size.height / 3.66)
                if moving.speed < 16 {
                    moving.speed += 0.15
                }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if moving.speed > 0 {
            if (contact.bodyA.categoryBitMask & colliderType.score.rawValue) == colliderType.score.rawValue ||  (contact.bodyB.categoryBitMask & colliderType.score.rawValue) == colliderType.score.rawValue {
                ...
                //var showHighScore = userDefaults.doubleForKey("Highest Score")
